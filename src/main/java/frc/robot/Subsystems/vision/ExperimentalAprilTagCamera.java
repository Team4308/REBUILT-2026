...
/**
 * EXPERIMENTAL: An implementation of an AprilTag camera that consumes raw data
 * arrays
 * from a Python backend via NetworkTables instead of using standard
...
public class ExperimentalAprilTagCamera extends AprilTagCamera {
     ...
     */
    public ExperimentalAprilTagCamera(String name, Transform3d robotToCam, AprilTagFieldLayout layout,
     ...
     * Expects a specific protocol: [Timestamp, Count, Target1_Data...,
     * Target2_Data...].
     *
    ...
    public PhotonPipelineResult getLatestResult() {
        ...
        for (int i = 0; i < targetCount; i++) {
            ...
            List<TargetCorner> corners = List.of(
                    ...
                    new TargetCorner(data[base + 10], data[base + 11]),
                    new TargetCorner(data[base + 12], data[base + 13]));
            targets.add(new PhotonTrackedTarget(data[base + 1], data[base + 2], data[base + 3], data[base + 4],
                    (int) data[base + 0], -1, 0.0f, new Transform3d(), new Transform3d(), data[base + 5], corners,
                    corners));
        }
